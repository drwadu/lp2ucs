    ...
    def simple_cycles(self, ctl, lp: str, mapping, verbose: bool = False):
        ...
        for r in grd:
            if isinstance(r, clingoext.ClingoRule):
                for a in r.head:
                    ...
                    #pbody = set(map(lambda x: x > 0, r.body))
        ...
        if verbose:
            ...
            for sl in simple_loops:
                ...
                # write external supports
                print(
                    *set().union(
                        *[
                            set(map(lambda y: -mapping[sym[y]], body))
                            for rule_bodies in map(lambda x: supps[x], sl)
                            for body in rule_bodies if not [b for b in body if b in sl]
                        ]
                    )
                )
