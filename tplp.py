...
class Program(object):
    def __init__(self, clingo_control):
        ...
        ###
        # print(self.ground_program(clingo_control).__dict__)
        ##for s in self.ground_program(clingo_control):
        ##    if isinstance(s, clingoext.ClingoOutputAtom):
        ##        print(f"{str(s.symbol)}.")

        ##    elif isinstance(s, clingoext.ClingoRule):
        ##        #print(f"{str(s)}.")
        ##        for h in s.head:
        ##            for sa in clingo_control.symbolic_atoms:
        ##                if h == sa.literal:
        ##                    print(sa.symbol, end=" ")
        ##        print(":- ", end=" ")
        ##        for b in s.body:
        ...
        ##                if b == sa.literal:
        ##                    print(sa.symbol, end=" ")
        ##                #print(sa.literal, sa.symbol)
        ##        print(".")

        ##supps = dict()
        ##dep = nx.DiGraph()
        ##for r in self.ground_program(clingo_control):
        ##    if isinstance(r,clingoext.ClingoRule):
        ##        for a in r.head:
        ##            if supps.get(a,None):
        ...
        ##print(dep,supps)
        ##simple_loops = set(map(frozenset, nx.simple_cycles(dep)))
        ##for sl in simple_loops:
        ##    for a in sl:
        ##        for sa in clingo_control.symbolic_atoms:
        ##            if a == sa.literal:
        ##                print(sa.symbol, end=" ")
    ...
    def simple_cycles(self, ctl, lp: str, mapping, verbose: bool = False):
        # glp = ctl.ground_program
        grd = self.ground_program(ctl)
        sym = dict()
        d = None
        ...
        for r in grd:
            if isinstance(r, clingoext.ClingoRule):
                for a in r.head:
                    for l in ctl.symbolic_atoms:
                        if l.literal == a:
                            sym[a] = str(l.symbol)
                            sym_i[str(l.symbol)] = a
                    #if supps.get(a, None):
                    #    supps[a].extend(r.body)
                    #else:
                    #    supps[a] = r.body
                    if not supps.get(a, None):
                        supps[a] = []
                    for b in r.body:
                        if b > 0:
                            supps[a].append(b)
        ...
        #for (x,y) in supps.items():
        #    print( mapping[sym.get(x,"edge(1,2)")],sym.get(x, None), "|", *map(lambda z: mapping[sym.get(z,"edge(1,2)")] if z > 0 else -mapping[sym.get(-z,"edge(1,2)")], y))
        #### custom
###        for k, v in sym.items():
###            if v.startswith("reach"):
###                n = int(v.split("(")[-1].replace(")", ""))
###                for k_, v_ in sym.items():
###                    if v_.startswith("select") and v_.endswith(f"{n})"):
###                        dep.add_edge(k_, k)
###                        if supps.get(k, None):
###                            supps[k].append(k_)
###                        else:
...
###                n = int(v.split("(")[-1].split(",")[0])
###                for k_, v_ in sym.items():
###                    if v_.startswith("reach") and v_.endswith(f"{n})"):
###                        dep.add_edge(k_, k)
###                        if supps.get(k, None):
###                            supps[k].append(k_)
###                        else:
###                            supps[k] = [k_]
###                #k_ = sym_i.get(v.replace("select", "edge"), None)
###                #if k_:
###                #    dep.add_edge(k_, k)
###                    # supps[k].append(k_)
###                # k_ = sym_i.get(v.replace("select", "nots"), None)
###                # if k_:
###                #    supps[k].append(-k_)
###
        ### end
        ...
        #    print(sym.get(a,None),sym.get(b,None))
        # print("###")
        if verbose:
            # print(dep.edges)
            for sl in simple_loops:
                print("cycle", *sl)
                print("cycle: ", *map(lambda a: sym[a] if a > 0 else "-" + sym[-a], sl))
                for a in sl:
                    print(f"supports of {a}", supps[a])
                    print(
                        f"supports of {sym[a] if a > 0 else '-'+sym[-a]}:",
                        *map(
                            lambda a: sym[a] if a > 0 else "-" + sym[-a],
                            filter(lambda b: b not in sl, supps[a]),
                        ),
                    )
                    # print(*supps[a],end="")
                print()
            ...
            for sl in simple_loops:
                print(*map(lambda x: mapping[sym[x]], sl), end=" ")
                print(
                    *map(
                        lambda b: -b,
                        set().union(
                            *[
                                set(
                                    map(
                                        lambda x: -mapping[sym[abs(x)]]
                                        if x < 0
                                        else mapping[sym[x]],
                                        filter(lambda c: abs(c) not in sl, supps[a]),
                                    )
                                )
                                for a in sl
                            ]
    ...
    def ground_program(self, clingo_control):
        return clingo_control.ground_program
    ...
    def _new_normalize(self, clingo_control):
        program = self.remove_tautologies(clingo_control)
        self._program = []
        _atomToVertex = {}  # htd wants succinct numbering of vertices / no holes
        _vertexToAtom = {}  # inverse mapping of _atomToVertex
    ...
    def grd(self):
        for r in self._program:
            print(
                r,
                *list(map(lambda i: self._nameMap[i], r.head)),
                " :- ",
                *list(
                    map(
                        lambda i: "not " + self._nameMap[abs(i)]
                        if i < 0
                        else self._nameMap[i],
                        r.body,
                    )
                ),
            )
...
if __name__ == "__main__":
    ...
    # lp.simple_cycles(ctl, s, True)
    lp.simple_cycles(ctl, s, CCG_MAPPINGS)
    exit(0)
    ...
    def write(loop):
        print(
            *map(lambda a: CCG_MAPPINGS[LP_MAPPINGS[a]], loop), end=" "
        )  # write cyclic atoms
        # print(*map(lambda a: atomify(a), loop), end=" ") # write cyclic atoms
        external_supports = set().union(
            *map(
                set,
                [list(filter(lambda l: not abs(l) in loop, supports[a])) for a in loop],
            )
        )
        print(
            *map(
                lambda l: -CCG_MAPPINGS[LP_MAPPINGS[l]]
                if l > 0
                else CCG_MAPPINGS[LP_MAPPINGS[-l]],
                external_supports,
            )
        )
